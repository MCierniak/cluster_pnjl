...
def gcp_real_a0(T : float, mu : float, Phi : complex, Phib : complex, bmass : float, thmass : float, d : float, Ni : float, L : float, T_crit : float, **kwargs):
    ...
    def mass_integrand2(_m, _p, _T2, _bmass, _thmass, _Ni, _L, _T_crit, key2):
        yp = {}
        yp["y_val"], yp["y_status"] = pnjl.aux_functions.y_plus(_p, _T2, 0.0, _m, 0.0, 1.0, **key2)

        hz = 0.5
        nlambda = _Ni * _L
        nlambda2 = 5.8 * nlambda

        frac1 = _m / nlambda
        frac2 = _thmass / nlambda

        heavi5 = numpy.heaviside(nlambda2 - 10.0 * (_T2 - _T_crit), hz)
        heavi7 = numpy.heaviside(math.pi * (_m - _bmass), hz)
        heavi8 = numpy.heaviside(math.pi * (((_bmass + 10.0 * (_T2 - _T_crit)) / nlambda) + 1.0 - (_m / nlambda)), hz)
        
        arccos_in = (2 * _m / (10.0 * (_T2 - _T_crit) + nlambda)) - ((2.0 * _bmass + 10.0 * (_T2 - _T_crit) + nlambda) / (10.0 * (_T2 - _T_crit) + nlambda))
        cos_in = math.pi * arccos_in / 2.0
        factor = 1.8 * ((nlambda2 - 10.0 * (_T2 - _T_crit)) / nlambda2)

        arccos_el2 = math.acos(arccos_in)

        continuum_factor = heavi5 * heavi7 * heavi8 * math.cos(cos_in) * (arccos_el2 / math.pi) * factor
        return (_m / pnjl.aux_functions.En(_p, _m)) * pnjl.thermo.distributions.f_baryon_singlet(**yp) * continuum_factor
    def integrand2(p, _T, _Mi, _Mth, _ni, _l, _T_c, key):
        inner_int, _ = scipy.integrate.quad(mass_integrand2, _Mi, _Mi + 10.0 * (_T - T_trans) + _ni * _l, args = (p, _T, _Mi, _Mth, _ni, _l, _T_c, key))
        return (p ** 2) * inner_int
...
def gcp_real(T : float, mu : float, Phi : complex, Phib : complex, bmass : float, thmass : float, a : int, b : int, d : float, Ni : float, L : float, T_crit : float, **kwargs) -> float:
    #
    return {(0, 0) : gcp_real_a0, (2, 0) : gcp_real_a2, (3, 0) : gcp_real_a3, (4, 0) : gcp_real_a4, (5, 0) : gcp_real_a5, (6, 0) : gcp_real_a6, (1, -1) : gcp_real_a1_bm1}[a, b](T, mu, Phi, Phib, bmass, thmass, d, Ni, L, T_crit, **kwargs)
def gcp_imag(T : float, mu : float, Phi : complex, Phib : complex, bmass : float, thmass : float, a : int, b : int, d : float, Ni : float, L : float, T_crit : float, **kwargs) -> float:
    #
    return {(0, 0) : gcp_imag_a0, (2, 0) : gcp_imag_a2, (3, 0) : gcp_imag_a3, (4, 0) : gcp_imag_a4, (5, 0) : gcp_imag_a5, (6, 0) : gcp_imag_a6, (1, -1) : gcp_imag_a1_bm1}[a](T, mu, Phi, Phib, bmass, thmass, d, Ni, L, T_crit, **kwargs)

#Extensive thermodynamic properties

def pressure(T : float, mu : float, Phi : complex, Phib : complex, bmass : float, thmass : float, a : int, b : int, d : float, Ni : float, L : float, T_crit : float, **kwargs):
    #
    return -gcp_real(T, mu, Phi, Phib, bmass, thmass, a, b, d, Ni, L, T_crit, **kwargs)
...
def bdensity(T : float, mu_vec : list, Phi_vec : list, Phib_vec : list, bmass : float, thmass : list, a : int, b : int, d : float, Ni : float, L : float, T_crit : float, **kwargs):
    ...
    if len(mu_vec) == len(Phi_vec) and len(mu_vec) == len(Phib_vec) and len(mu_vec) == len(thmass):
        if len(mu_vec) == 4 and numpy.all(mu_vec[i] > mu_vec[i + 1] for i, el in enumerate(mu_vec[:-1])):
            h = mu_vec[0] - mu_vec[1]
            p_vec = [pressure(T, mu_el, Phi_el, Phib_el, bmass, Mth_el, a, b, d, Ni, L, T_crit, **kwargs) for mu_el, Phi_el, Phib_el, Mth_el in zip(mu_vec, Phi_vec, Phib_vec, thmass)]
...
def sdensity(T_vec : list, mu : float, Phi_vec : list, Phib_vec : list, bmass : float, thmass : list, a : int, b : int, d : float, Ni : float, L : float, T_crit : float, **kwargs):
    ...
    if len(T_vec) == len(Phi_vec) and len(T_vec) == len(Phib_vec) and len(T_vec) == len(thmass):
        if len(T_vec) == 4 and numpy.all(T_vec[i] > T_vec[i + 1] for i, el in enumerate(T_vec[:-1])):
            h = T_vec[0] - T_vec[1]
            p_vec = [pressure(T_el, mu, Phi_el, Phib_el, bmass, Mth_el, a, b, d, Ni, L, T_crit, **kwargs) for T_el, Phi_el, Phib_el, Mth_el in zip(T_vec, Phi_vec, Phib_vec, thmass)]
            return (1.0 / 3.0) * (p_vec[3] - 8.0 * p_vec[2] + 8.0 * p_vec[1] - p_vec[0]) / (12.0 * h)
